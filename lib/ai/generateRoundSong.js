...
const { expandRevealLinesToSong } = require("./lyricFillerProvider");
...
function buildInitialGeneratedSongState(lyricsLines = [], revealLines = []) {
  return {
    status: "idle",
    audioUrl: null,
    mimeType: null,
...
async function generateRoundSongAssets(roundState) {
  const baseRevealLines = Array.isArray(roundState.revealLines)
    ? roundState.revealLines
    : [];
  const roundId =
    roundState.roundId ||
  ...
  try {
    const expandedRevealResult = await expandRevealLinesToSong({
      title: roundState.title,
      stylePrompt: roundState.stylePrompt,
      revealLines: baseRevealLines,
      targetDurationSec: durationSec
    });
    const performanceRevealLines = expandedRevealResult.revealLines;
    const lyricsLines = performanceRevealLines.map((line) => line.text);

    const musicResult = await generateSongFromLyrics({
      roundId,
      title: roundState.title,
      stylePrompt: roundState.stylePrompt,
      lyricsLines: performanceRevealLines,
      durationSec
    });
    ...
    try {
      alignmentResult = await alignLyricsToAudio({
        ...
        transcriptText: musicResult.generatedLyricsText || lyricsLines.join("\n"),
        lyricsLines
      });
    ...
    metadata.fillerProvider = expandedRevealResult.fillerProvider;
    metadata.fillerPrompt = expandedRevealResult.fillerPrompt;
    ...
    metadata.performanceRevealLines = performanceRevealLines;
    metadata.finalLyricLines = lyricsLines;
    ...
    roundState.generatedSong = {
      ...
      lyricsLines,
      revealLines: performanceRevealLines,
      lineTimings,
      wordTimings,
      ...
      roundId,
      usedFiller: expandedRevealResult.usedFiller,
      fillerProvider: expandedRevealResult.fillerProvider,
      fillerPrompt: expandedRevealResult.fillerPrompt
    };
