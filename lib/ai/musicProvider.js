...
function shouldUseMockFallback() {
  return String(process.env.MOCK_AI_FALLBACK || "true").toLowerCase() !== "false";
}
...
async function callGoogleLyria({
  ...
  try {
    ...
    const quotaOrAuthFailure =
      /RESOURCE_EXHAUSTED|quota exceeded|429|rate[- ]?limit|billing|permission/i.test(
        message
      );

    if (shouldUseMockFallback() && quotaOrAuthFailure) {
      console.warn(
        "Google Lyria request failed. Falling back to deterministic mock audio:",
        message
      );

      const mockResult = createMockSong({
        roundId,
        title,
        stylePrompt,
        lyricsLines,
      ...
      return {
        ...mockResult,
        provider: "mock",
        promptUsed: `${promptUsed}\n\n[FALLBACK]\nOriginal Lyria request failed: ${message}`
      };
    }

    throw new Error(`Google Lyria generation failed: ${message}`);
  }
}
