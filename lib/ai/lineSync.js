...
function findBestTokenMatches(tokens, normalizedWords, searchCursor) {
  ...
  for (const token of tokens) {
    ...
    const lookaheadLimit = Math.min(
      ...
      cursor + Math.max(24, tokens.length * 14)
    );
  ...
  const coverage = tokens.length > 0 ? matchedIndices.length / tokens.length : 0;
...
function deriveLineTimings({ lyricsLines, words, totalDurationMs }) {
  ...
  const lines = safeLyricsLines.map((text) => {
    ...
    const matchResult = findBestTokenMatches(tokens, normalizedWords, searchCursor);
    const minimumMatches =
      tokens.length <= 2
        ? Math.min(tokens.length, 1)
        : Math.max(2, Math.ceil(tokens.length * 0.45));
    const hasUsableMatch = matchResult.matchedIndices.length >= minimumMatches;
    const matchedIndices = hasUsableMatch ? matchResult.matchedIndices : [];

    if (hasUsableMatch) {
      searchCursor = matchResult.nextCursor;
    }
    ...
    return {
      ...
      _coverage: matchResult.coverage
    };
  });
